...
class Diffusion(object):
    ...
    def simplified_ddnm_plus(self, model, cls_fn):
        ...
        if args.deg =='colorization':
            ...
            # mask_2 = torch.from_numpy(loaded).to(self.device)
            mask = self.config.mask.to(self.device)
            A = lambda z: z*mask
            Ap = A
        ...
        for x_orig, classes in pbar:
            ...
            # os.makedirs(os.path.join('/home/mantasu/projects/refrem/configs', "Apy"), exist_ok=True)
            # for i in range(len(Apy)):
            #     tvu.save_image(
            #         inverse_data_transform(config, Apy[i]),
            #         os.path.join('/home/mantasu/projects/refrem/configs', f"Apy/Apy_{idx_so_far + i}.png")
            #     )
            #     tvu.save_image(
            #         inverse_data_transform(config, x_orig[i]),
            #         os.path.join('/home/mantasu/projects/refrem/configs', f"Apy/orig_{idx_so_far + i}.png")
            #     )
